...
sub child_stdout {
    ...
    my $file = 'LOG_'.$Monatstag.'_'.$Monat.'_'.$Jahr.'.log';
...
sub stop_gameserver {
    ...
    $_[HEAP]->{wheel}->put("echo \"Attention server is going down for reboot.\"");
    $_[HEAP]->{wheel}->put("echo \"Attention server is going down for reboot.\"");
    $_[HEAP]->{wheel}->put("echo \"Attention server is going down for reboot.\"");
...
sub restart {
    ...
    POE::Kernel->delay('_start_gameserver' => 3);
...
sub install {
    ...
    my $folder = $PACKAGE_DIR . "/Gameserver/Counter-Strike-Source/";
...
sub _remove {
    ...
    if ( !rmtree($path) ) {
        $logger->warn("Couldn't delete directofrom gameserver in $path: $!.");
